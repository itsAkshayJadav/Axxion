...
export async function POST(req) {
  try {
    ...
    const inquiryEmail = getEnv("INQUIRY_TO_EMAIL");

    if (smtpHost && smtpPort && smtpUser && smtpPass && inquiryEmail) {
      const host = getRequiredEnv("SMTP_HOST");
      const port = Number(getRequiredEnv("SMTP_PORT"));
      const user = getRequiredEnv("SMTP_USER");
      const pass = getRequiredEnv("SMTP_PASS");
      const to = getRequiredEnv("INQUIRY_TO_EMAIL");
      const from = getEnv("INQUIRY_FROM_EMAIL") || user;
      ...
      await transporter.sendMail({
        ...
        text: [
          ...
          `Saved inquiry ID: ${savedInquiry.id}`,
          "",
          "Project details:",
          projectDetails,
        ].join("\n"),
        html: `
          <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #0f172a;">
            ...
            <p><strong>Saved inquiry ID:</strong> ${escapeHtml(savedInquiry.id)}</p>
